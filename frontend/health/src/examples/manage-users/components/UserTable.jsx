    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="bg-gray-100 text-gray-600">
          <tr>
            <th className="px-4 py-3 text-left">{t('Patient Name')}</th>
            ...
            <tr key={p.id} className="border-t border-gray-200 hover:bg-gray-50">
              ...
              <td className="px-4 py-3">
                <div className="flex items-center gap-4 text-sm">
                  <button onClick={() => onEdit?.(p)} className="text-blue-600 hover:underline inline-flex items-center gap-2"><FaEdit /> {t('Edit')}</button>
                  <button onClick={() => onToggle?.(p)} className={`inline-flex items-center gap-2 ${p.status === 'Active' ? 'text-red-600' : 'text-green-600'}`}>{p.status === 'Active' ? t('Deactivate') : t('Activate')}</button>
                  <button onClick={() => onDelete?.(p)} className="text-red-500">{t('Delete')}</button>
                </div>
              </td>
            </tr>
          ))}
